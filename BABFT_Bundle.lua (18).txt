...
Modules.Crypto = (function()
    ...
    function Crypto.encrypt(str)
        ...
        for _, code in utf8.codes(key) do
            if code < 128 then
                table.insert(keyBytes, code)
            elseif code < 2048 then
                table.insert(keyBytes, 192 + math.floor(code / 64))
                table.insert(keyBytes, 128 + (code % 64))
            else
                table.insert(keyBytes, 224 + math.floor(code / 4096))
                table.insert(keyBytes, 128 + (math.floor(code / 64) % 64))
                table.insert(keyBytes, 128 + (code % 64))
            end
        end
        ...
        for _, code in utf8.codes(str) do
            if code < 128 then
                table.insert(strBytes, code)
            elseif code < 2048 then
                table.insert(strBytes, 192 + math.floor(code / 64))
                table.insert(strBytes, 128 + (code % 64))
            else
                table.insert(strBytes, 224 + math.floor(code / 4096))
                table.insert(strBytes, 128 + (math.floor(code / 64) % 64))
                table.insert(strBytes, 128 + (code % 64))
            end
        end

        for i = 1, #strBytes do
            local charByte = strBytes[i]
    ...
    function RemoteCompute.request(action, data)
        ...
        if success and type(response) == "table" and response.StatusCode == 200 then
            ...
            if not decodeSuccess then
                warn("Erro ao decodificar JSON:", decoded)
                return nil
            end
