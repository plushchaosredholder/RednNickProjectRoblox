...
Modules.Crypto = (function()
    local Crypto = {}
    local key = "BABFT_SECURE_KEY_2026"
...
Modules.RemoteCompute = (function()
    ...
    local SUPABASE_KEY = "sb_publishable_aKgXreraJmKEipNlqEH3ew_4-5AEBZy"
    ...
    function RemoteCompute.request(action, data)
        ...
        if success and response and response.StatusCode == 200 then
            ...
            local decryptedResponse = Crypto.decrypt(response.Body)
            local decoded = HttpService:JSONDecode(decryptedResponse)
            ...
            warn("Erro no Supabase:", response and response.StatusCode, response and response.Body)
        end
        return nil
    end
